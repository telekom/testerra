...
| {browser_setting} | | You can combine browser type and version as single string like `firefox:65`
| {browser_proxy} | na. | Sets the default proxy for all browsers and protocols.
| {browser_proxy_default} | false | Use the system's default http proxy configuration for all browsers and protocols. |
