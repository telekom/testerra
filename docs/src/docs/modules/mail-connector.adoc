...
General methods for accessing and adjusting the server parameters are provided within this class.

== AbstractInboxConnector
This abstract class extends the AbstractMailConnector by adding methods for working with mailboxes like reading, moving and deleting messages from the inbox as well as means to poll the inbox until an email is received.
