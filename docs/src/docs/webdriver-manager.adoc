...
With a local session you don't need a remote Selenium server.
Testerra is using its build-in Selenium libary to create a new browser session.
...
TIP: Use as often as possible a remote Selenium server, also for development.
So your project is independed from any Webdriver configuration and needed Webdriver binary files.

[TIP]
======
You can also use a simple way to define browser type and version:
...
To switch windows in a easy way Testerra provides some methods.

[source, jaava]
----
// Switch to a window by matching title
// Use `withoutWait` if you don't want to use retry
WebDriverUtils.findWindowAndSwitchTo(String windowTitle, boolean withoutWait)
----
