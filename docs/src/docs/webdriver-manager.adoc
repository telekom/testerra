...
WebDriverManager uses the standard Selenium Webdriver but it is much simpler to configure your session or create a new remote Selenium session.
...
With `wdm.closewindows.aftertestmethods` you can control this behaviour.

At the end of the test run Testerra alwawys calls `WebDriverManager.forceShutdown()` to cleanup all sessions.
====
...
String uuid WebDriverManager.makeSessionExclusive(WebDriver driver)
WebDriverManager.shutdownExclusiveSession(String key)
