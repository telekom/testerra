...
To control the threshold, you can use the following properties.

* `{layoutcheck_pixel_rgb_deviation_percent}` defines, which color difference in percentage of a single pixel is count as a match. You can use that value to handle slightly changes in color or alpha values.
* `{layoutcheck_match_threshold}` is the overall percentage amount of pixels that need to match, to pass the layout check based test.
