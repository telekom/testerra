...
// Get the session key
String key1 = webDriverManager.getSessionKey(driver);        // key1 contains 'default'

String key2 = webDriverManager.getSessionKey(driverWindow2); // key2 contains 'window2'
----
...
[NOTE]
====
Testerra shuts down the WebDriver at the end of every test method.

At the end of the test run Testerra always calls `IWebDriverManager.shutdownAllSessions()` to cleanup all sessions.
...
public void test1() {
    WebDriver driver = webDriverManager.getWebDriver();
    uuid = webDriverManager.createExclusiveSessionKey(driver);
}
...
@AfterTest
public void cleanup() {
    webDriverManager.shutdownSession(uuid);
}
----
=====
