...
* at runtime by `webDriverManager.getDefaultConfig()` (only browser session behaviour)
...
webDriverManager.getDefaultConfig().setShutdownSessionAfterTestMethod(true);

// tt.wdm.closewindows.onfailure
webDriverManager.getDefaultConfig().setShutdownSessionOnFailure(false);

// tt.browser.maximize
webDriverManager.getDefaultConfig().setMaximizeViewport(true);

// tt.webdriver.mode
webDriverManager.getDefaultConfig().setWebDriverMode(WebDriverMode.remote);
----

[NOTE]
======
`webDriverManager.getDefaultConfig().setShutdownSessions()` controls closing of browser windows after every test method in general,
...
[source,java]
----
webDriverManager.getDefaultConfig().reset();
----
