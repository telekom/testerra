...
Perform decisions on occurrence
```
if (guiElement.displayed().perhaps().isTrue()) {
...
```java
guiElement.css("display").is("hidden"); <1>
...
. If text does not contain "Something", start over with 1.
...
==== More consistence, less complexity and more features
...
Testerra is built on the Page Object pattern in mind. The new API makes it harder for you and your team to
break out and keeps you on track, even if your project contains hundreds of Pages and thousands of Tests.
...
. StringPropertyAssertion allows you to perform assertions on strings like `string.contains("Something")`
. QuantifiedPropertyAssertion allows you to perform assertions on quantified values like `value.isBetween(-2,3)`
. BinaryPropertyAssertion allows to assert if an value is boolean or a string that represents a boolean value with `displayed.isTrue()`
...
. Return a generic StringAssertion with the given file name
