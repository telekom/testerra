...
Using a Seleniumd Grid based on https://github.com/aerokube/selenoid[Selenoid] this module provides access to videos and VNC streams.
...
After finishing the test run Testerra collects the video files and add them to the report.


.You will find all video files in the method detail view in a separate tab.
image::selenoid_connector_report_video_tab.png[]

IMPORTANT: Testerra add video files only to failed methods.
...
|{screencaster_active} | true | Video files will be collected in failure case of test method and for exclusive sessions.
