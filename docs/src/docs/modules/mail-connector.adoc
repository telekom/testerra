...
# TIMING
# sets the timeout between polls. default is 50s
POLLING_TIMER_SECONDS=10
# sets the maximum number of polls, default is 20
MAX_READ_TRIES=20
----
...
reading, moving and deleting messages from the inbox as well as means to poll the inbox until an email is received.
...
== SMTPMailConnector
This entity allows sending emails via the SMTP protocol. As a descendent of AbstractMailConnector it has all its functions.
...
String subject = "test";
String content = "Testerra Testmail"
...
MimeMessage decryptedMsg = MailUtils.decryptMessageWithKeystore(encryptedMsg, session, pahtKeyStore,
        password);
// verify received content is equal to sent one
String actualText = ((Multipart) decryptedMsg.getContent()).getBodyPart(0).getContent().toString();
