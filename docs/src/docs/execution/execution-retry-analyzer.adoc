...
[#NoRetry]
== @NoRetry
With this annoation the Retry analyzers won't retry these methods if previously failed. This is characteristically shown in the report with the badge `NoRetry`.
