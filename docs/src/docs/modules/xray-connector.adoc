...
Current release: image:https://img.shields.io/bintray/v/testerra-io/Testerra/eu.tsystems.mms.tic.testerra:xray-connector?label=Testerra%20Xray%20connector[Bintray]
...
public class TestMethodNameMapper implements XrayMapper {
    ...
    public JqlQuery resultToXrayTest(ITestResult testNgResult) {
        ...
        final String methodName = testNgResult.getMethod().getMethodName();
        if (parameters.length > 0) {
            final String summary = String.format("%s with %s", methodName, parameters[0]);
            return JqlQuery.create()
                    .addCondition(new SummaryContainsExact(summary))
                    .build();
        } else {
            return JqlQuery.create()
                    .addCondition(new SummaryContainsExact(methodName))
                    .build();
        }
    }
...
[NOTE]
====
The default `EmptyMapper` will return `null`.
...
As you may noticed in the code examples above we provided a `EmptyTestExecutionUpdates` as implementation for `XrayTestExecutionUpdates` in our `FooXrayResultsSynchronizer`.
