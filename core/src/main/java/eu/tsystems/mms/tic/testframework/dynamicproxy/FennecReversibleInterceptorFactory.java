 ...
 */
public final class FennecReversibleInterceptorFactory<T> {
    protected final Map<T, FennecReversibleInterceptor<T>> proxied;

    public FennecReversibleInterceptorFactory() {
        proxied = new HashMap<>();
    }
    ...
    public T getUnproxied(Object proxiedObject) {
        FennecReversibleInterceptor<T> proxy = proxied.get(proxiedObject);
        return (proxy == null) ? (null) : proxy.getUnproxiedObject();
    }

    public T proxify(T unproxiedObject, IDefaultMethodWrapper wrapper) {
        FennecReversibleInterceptor<T> proxy = new FennecReversibleInterceptor<>(unproxiedObject, wrapper);
