...
uiElement.text().matches("^Hello\\s.orld").isTrue(); <2>
...
```java
uiElement.displayed().isTrue("This element should be displayed, which is ${actual}");
```
...
```java
Control.withElementTimeout(1, () -> uiElement.displayed().isTrue());
```
...
```java
UiElement div = find(Locate.by(XPath.node("div")));
```
