...
```
if (uiElement.waitFor().displayed().isTrue()) {
...
uiElement.text().map(value -> value.toLowerCase()).matches("^hello\\s.orld").isTrue(); <2,3>
...
```java
uiElement.displayed().isTrue("This element should be displayed");
```
...
body.bounds().contains(nav).isTrue();
```
[NOTE]
...
```java
Control.collectAssertions(() -> uiElement.displayed().isTrue());
```
...
```java
Control.nonFunctional(() -> uiElement.displayed().isTrue());
```
...
```java
Control.withTimeout(1, () -> uiElement.displayed().isTrue());
```
...
Control.retryFor(10).withTimeout(0, () -> {
    ...
    guiElement.visible(false).isTrue();
});
```
