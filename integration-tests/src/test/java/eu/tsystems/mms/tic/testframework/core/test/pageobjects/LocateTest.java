...
public class LocateTest extends AbstractTestSitesTest implements UiElementCreator {
    ...
    public void locateByClass() {
        UiElement div = findByXPath(XPath.from("div").classNames("element", "multiple"));
    ...
    public void locateByText_fails() {
        ...
        UiElement a = findByXPath(XPath.from("a").text(textToFind));
    ...
    public void locateByTextContains_fails() {
        UiElement realA = findByQa("action/linkWithFormattedText");
        UiElement a = findByXPath(XPath.from("a").textContains(textToFind));
        a.text().is(realA.text().getActual());
    }
    ...
    public void locateByWords() {
        UiElement realA = findByQa("action/linkWithFormattedText");
        UiElement a = findByXPath(XPath.from("a").textWords(textToFind));
        a.text().is(realA.text().getActual());
    }
    ...
    public void testLocateByClassWord() {
        UiElement div = findByXPath(XPath.from("*").classNames("header","large"));
        div.text().is("You found me");
    }
    ...
    public void testLocatePosition() {
        UiElement li;
        li = findByXPath(XPath.from("ul").classNames("list-group").select("li",1));
        li.text().is("Affe");

        li = findByXPath(XPath.from("ul").classNames("list-group").select("li",2));
        li.text().is("Katze");
    ...
    public void testLocateByWords() {
        UiElement div = findByXPath(XPath.from("*").textWords("Login here"));
