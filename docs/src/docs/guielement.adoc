...
GuiElements are not, but based on the *Selenium WebElement* and add more functionality to them. Since a GuiElement is just a pointer to a locator, it's using the same definition as WebElements `By` (https://seleniumhq.github.io/selenium/docs/api/java/org/openqa/selenium/By.html[Selenium docs]).
