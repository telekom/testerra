...
public class ReportingDetailsTest extends AbstractReportingTest {
    ...
    @Test
    public void testAddScreenshotsToMethod() throws Exception {
        MethodContext methodContext = ExecutionContextController.getCurrentMethodContext();
        String reportSCFolder = ReportUtils.getScreenshotsPath();
        for (int i = 0; i < 40; i++) {
            Screenshot screenshot = new Screenshot();
            String scFileName = i + "_10.png";
            FileUtils.copyFile(new File("src/test/resources/report/10.PNG"), new File(reportSCFolder, scFileName));
            screenshot.filename = scFileName;
            methodContext.screenshots.add(screenshot);
        }
    }
