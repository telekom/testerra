...
| {layoutcheck_internal_parameter1}       | 5 | Max distance for grouping multiple matches as single match (distance of matches < {layoutcheck_internal_parameter1} are marked as single match)
| {layoutcheck_internal_parameter2}       | 10 | Minimal difference in size of the reference and actual image, to consider the reference image as sub image.
| {layoutcheck_internal_parameter4}       | 14 | Max distance between matches until a warning message about the parameter setting is logged. (matches have distance < {layoutcheck_internal_parameter4} a warning is logged)
...
| {layoutcheck_ignore_mssing_elements}       | - | when true ignore missing elements, which mean no match was found for a template.
...
| {layoutcheck_internal_parameter3}       | - | -
