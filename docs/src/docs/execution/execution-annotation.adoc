...
== @NoRetry
This marks a test method as `No Retry`, which is characteristically shown with this text the report. <<Retry analyzer, RetryAnalyzers>> won't retry these methods if previously failed. Customization is possible with the two attributes name and color in the annotation.
