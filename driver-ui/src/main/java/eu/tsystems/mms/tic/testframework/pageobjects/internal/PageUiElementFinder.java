...
/**
 * Default implementation of {@link UiElementFinder} for pages
 * @author Mike Reiche
 */
public class PageUiElementFinder implements UiElementFinder {
    ...
    public UiElement find(Locator locator) {
        UiElement uiElement = uiElementFactory.createWithWebDriver(getWebDriver(), locator);
        if (uiElement instanceof NameableChild) {
            ((NameableChild)uiElement).setParent(this.page);
        }
        return uiElement;
    }
    ...
    public UiElement newEmpty() {
        return new EmptyUiElement(this.page);
    }
